...
class RequestDeduplicatorClass {
  ...
  private generateRequestKey(
    ...
    if (normalizedMethod === 'GET') {
      ...
      return `${normalizedMethod}:${normalizedUrl}:${authHeader.slice(-10)}`;
    }
    ...
    return `${normalizedMethod}:${normalizedUrl}:${dataHash}:${authHeader.slice(
      -10
    )}`;
  }
  ...
  private cleanup(): void {
    ...
    if (this.pendingRequests.size > this.config.maxPendingRequests) {
      ...
      const toRemove = sortedRequests.slice(
        0,
        this.pendingRequests.size - this.config.maxPendingRequests
      );
    ...
    if (
      ...
      (this.cache.size > 0 || this.pendingRequests.size > 0)
    ) {
      console.log(
        `RequestDeduplicator: Cache size: ${this.cache.size}, Pending: ${this.pendingRequests.size}`
      );
    }
  }
  ...
  clear(): void {
    ...
    if (this.config.debugMode) {
      console.log(
        'RequestDeduplicator: Cleared all cache and pending requests'
      );
    }
  }
